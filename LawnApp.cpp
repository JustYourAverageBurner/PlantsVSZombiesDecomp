...
int gIsPartnerBuild = false;
int gSlowMo = false;  //0x6A9EAA
int gFastMo = false;  //0x6A9EAB
...
//0x44F540
int LawnApp::WriteCurrentUserConfig()
{
	if (mPlayerInfo)
		mPlayerInfo->SaveDetails();
...
//0x44F560
void LawnApp::PreNewGame(GameMode theGameMode, int theLookForSavedGame)
{
	//if (NeedRegister())
	//{
	//	ShowGameSelector();
...
{
	//TOD_ASSERTmSeedChooserScreen == nullptr);
...
//0x4500D0
void LawnApp::DoNewOptions(int theFromGameSelector)
{
	//FinishModelessDialogs();
...
//0x450770
Dialog* LawnApp::DoDialog(int theDialogId, int isModal, const SexyString& theDialogHeader, const SexyString& theDialogLines, const SexyString& theDialogFooter, int theButtonMode)
{
...
Dialog* LawnApp::DoDialogDelay(int theDialogId, int isModal, const SexyString& theDialogHeader, const SexyString& theDialogLines, const SexyString& theDialogFooter, int theButtonMode)
{
...
//0x450930
void LawnApp::FinishUserDialog(int isYes)
{
...
//0x450AC0
void LawnApp::FinishCreateUserDialog(int isYes)
{
...
//0x450F40
void LawnApp::FinishConfirmDeleteUserDialog(int isYes)
{
	KillDialog(Dialogs::DIALOG_CONFIRMDELETEUSER);
	UserDialog* aUserDialog = (UserDialog*)GetDialog(Dialogs::DIALOG_USERDIALOG);
...
//0x451260
void LawnApp::FinishRenameUserDialog(int isYes)
{
	UserDialog* aUserDialog = (UserDialog*)GetDialog(Dialogs::DIALOG_USERDIALOG);
	if (!isYes)
	{
	...
	int isCurrentUser = mProfileMgr->GetProfile(anOldName) == mPlayerInfo;
...
void LawnApp::FinishCheatDialog(int isYes)
{
...
//0x451580
Dialog* LawnApp::NewDialog(int theDialogId, int isModal, const SexyString& theDialogHeader, const SexyString& theDialogLines, const SexyString& theDialogFooter, int theButtonMode)
{
...
//0x451630
int LawnApp::KillNewOptionsDialog()
{
	NewOptionsDialog* aNewOptionsDialog = (NewOptionsDialog*)GetDialog(Dialogs::DIALOG_NEWOPTIONS);
	if (aNewOptionsDialog == nullptr)
		return false;

	int wantWindowed = !aNewOptionsDialog->mFullscreenCheckbox->IsChecked();
	int want3D = aNewOptionsDialog->mHardwareAccelerationCheckbox->IsChecked();
...
//0x451800
int LawnApp::KillDialog(int theDialogId)
{
...
//0x451870
void LawnApp::ShowResourceError(int doExit)
{
...
//0x4522A0
int LawnApp::ChangeDirHook(const char* theIntendedPath)
{
	return false;
}
...
//0x4522C0
int LawnApp::DebugKeyDown(int theKey)
{
...
//0x452310
int LawnApp::UpdatePlayerProfileForFinishingLevel()
{
	int aUnlockedNewChallenge = false;
...
void LawnApp::CheckForGameEnd()
{
	...
	int aUnlockedNewChallenge = UpdatePlayerProfileForFinishingLevel();
...
//0x452F00
int LawnApp::OpenURL(const std::string& theURL, int shutdownOnOpen)
{
	DoDialog(
...
//0x453820
int LawnApp::IsShovelLevel()
{
	return mBoard && mGameMode == GameMode::GAMEMODE_CHALLENGE_SHOVEL;
}

//0x453840
int LawnApp::IsWallnutBowlingLevel()
{
	if (mBoard == nullptr)
		return false;
...
//0x4538C0
int LawnApp::IsLittleTroubleLevel()
{
...
//0x4538F0
int LawnApp::IsScaryPotterLevel()
{
...
//0x453920
int LawnApp::IsStormyNightLevel()
{
	if (mBoard == nullptr)
		return false;
...
//0x453950
int LawnApp::IsBungeeBlitzLevel()
{
	if (mBoard == nullptr)
		return false;
...
//0x453A00
int LawnApp::IsChallengeWithoutSeedBank()
{
	return 
...
{
	//TOD_ASSERTthePottedPlantIndex >= 0 && thePottedPlantIndex < mPlayerInfo->mNumPottedPlants);
	return &mPlayerInfo->mPottedPlant[thePottedPlantIndex];
}

//0x453A50
int LawnApp::UpdateApp()
{
	if (mCloseRequest)
	{
		Shutdown();
	...
	int updated = SexyAppBase::UpdateApp();
...
//0x453B20
int LawnApp::HasSeedType(SeedType theSeedType)
{
...
int LawnApp::SeedTypeAvailable(SeedType theSeedType)
{
...
//0x453D20
int LawnApp::AdvanceCrazyDaveText()
{
...
//0x454120
int LawnApp::HasBeatenChallenge(GameMode theGameMode)
{
	if (mPlayerInfo == nullptr)
		return false;

	int aChallengeIndex = theGameMode - GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1;
	//TOD_ASSERTaChallengeIndex >= 0 && aChallengeIndex < NUM_CHALLENGE_MODES);
...
//0x454170
int LawnApp::HasFinishedAdventure()
{
	return mPlayerInfo && mPlayerInfo->mFinishedAdventure > 0;
}

//0x454190
int LawnApp::IsFirstTimeAdventureMode()
{
...
{
	//TOD_ASSERTmCrazyDaveState == CRAZY_DAVE_OFF);
...
//0x455C90
int LawnApp::IsTrialStageLocked()
{
...
//0x4560E0
void LawnApp::SwitchScreenMode(int wantWindowed, int is3d, int force)
{
...
void LawnApp::BetaSubmit(int theAskForComments)
{
...
int LawnApp::CanDoRegisterDialog()
{
	return false;
}
